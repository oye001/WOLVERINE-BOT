...
// - Welcome messages -
function buildWelcomePrompt(name, mention) {
  return (
    "Write a short warm welcome for a new member joining the $WOLVE Telegram group. " +
    "Start the message with exactly this mention tag: " + mention + " " +
    "Then 2-3 lines max. Never start with the word Welcome. " +
    "Vary the opening, tone, energy, and structure completely every single time. " +
    "Sometimes be hype, sometimes calm, sometimes funny, sometimes motivating. " +
    "Make it feel natural like a real community member wrote it. " +
    "Always refer to the token as $WOLVE never $WOLVERINE. " +
    "Never mention the Telegram group link. " +
    (caUnlocked ? "You may naturally mention chart or how to buy." : "Do not mention CA or chart links yet.") +
...
// - Hype / ignore keyword filter (never call Groq for these) -
var HYPE_PATTERNS = [
  "ath", "moon", "mooning", "100x", "1000x", "lfg", "lets go", "let's go",
  "send it", "send her", "gm", "gn", "wagmi", "ngmi", "to the moon",
  "breaking", "pump", "pumping", "hold", "hodl", "lambo", "wen lambo",
  "based", "fr", "lol", "lmao", "haha", "nice", "wow", "ok", "okay",
  "hunt to", "100k", "1m mc", "market cap", "mc", "degen", "early",
  "bullish", "bearish", "buy the dip", "aping", "ape in", "just aped",
  "first", "ser", "king", "legend", "lets ride", "ride or die",
  "pamp", "pamp it", "rally", "ripping", "flying", "going up",
  "imagine", "they dont know", "they don't know", "claws out", "unleash",
];

function isHypeMessage(text) {
  var t = text.toLowerCase().trim();
  // Single word or very short = ignore
  if (t.split(" ").length <= 2 && !isCaRequest(t) && !isXRequest(t) && !isSocialsRequest(t) && !isBuyRequest(t)) return true;
  for (var i = 0; i < HYPE_PATTERNS.length; i++) {
    if (t.indexOf(HYPE_PATTERNS[i]) !== -1) return true;
  }
  return false;
}
...
bot.on("new_chat_members", async function(ctx) {
  ...
  for (var i = 0; i < members.length; i++) {
    ...
    var mention = member.username ? "@" + member.username : "<a href=\"tg://user?id=" + member.id + "\">" + name + "</a>";
    try {
      var welcomeText = await askGroqUnique(systemPrompt(caUnlocked), buildWelcomePrompt(name, mention));
      var wm = await ctx.reply(mention + " " + welcomeText, { parse_mode: "HTML" });
...
bot.on("message", async function(ctx) {
  ...
  // In group: filter hype first, then ask AI
  if (isGroup) {
    // Block obvious hype/casual messages before calling Groq
    if (isHypeMessage(text)) return;
    // Only call AI if message looks like a real project question
    var lowerText = text.toLowerCase();
    var isProjectQuestion = (
      lowerText.indexOf("?") !== -1 ||
      lowerText.indexOf("dev") !== -1 ||
      lowerText.indexOf("safe") !== -1 ||
      lowerText.indexOf("legit") !== -1 ||
      lowerText.indexOf("tax") !== -1 ||
      lowerText.indexOf("wallet") !== -1 ||
      lowerText.indexOf("renounce") !== -1 ||
      lowerText.indexOf("lock") !== -1 ||
      lowerText.indexOf("supply") !== -1 ||
      lowerText.indexOf("utility") !== -1 ||
      lowerText.indexOf("roadmap") !== -1 ||
      lowerText.indexOf("project") !== -1 ||
      lowerText.indexOf("team") !== -1 ||
      lowerText.indexOf("wolve") !== -1 ||
      lowerText.indexOf("wolverine") !== -1
    );
